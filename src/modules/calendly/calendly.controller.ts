...
export class CalendlyController {
    ...
    async oauthCallback(
      ...
      try {
        ...
        const redirectUrl = `${process.env.REMOTE_FRONTEND_DEV_URL}/calendly/${mentorId}?connect-calendly=success`;

        return res.redirect(redirectUrl);

      } catch (error) {
        const redirectUrl = `${process.env.REMOTE_FRONTEND_DEV_URL}/calendly/${mentorId}?connect-calendly=error`;
